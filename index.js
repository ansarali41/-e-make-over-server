...
// mongodb
client.connect(err => {
    const ordersCollection = client.db('eMakeOver').collection('orders');
    const reviewsCollection = client.db('eMakeOver').collection('reviews');
    const adminsCollection = client.db('eMakeOver').collection('admins');
    const productsCollection = client.db('eMakeOver').collection('products');
    ...
    app.post('/addProducts', (req, res) => {
        ...
        productsCollection.insertOne({ title, description, image, price }).then(result => {
            res.send(result.insertedCount > 0);
        });
    });
    ...
    app.delete('/deleteProduct/:id', (req, res) => {
        const id = req.params.id;
        productsCollection.deleteOne({ _id: objectId(req.params.id) }).then(result => {
            res.send(result.deletedCount > 0);
        });
    });
});
